...
// The SDK checkout location: the aggregator build clones it as a sibling of
// the staged demo; local dev can point anywhere via FC_RN_SDK_ROOT.
const sdkRoot = process.env.FC_RN_SDK_ROOT
  ? path.resolve(process.env.FC_RN_SDK_ROOT)
  : path.resolve(__dirname, '../fastcomments-react-native-sdk');
...
export default defineConfig(({ mode }) => ({
  // Served from fastcomments.com/tmp_demos/idcollab: assets must be relative.
  base: './',
  ...
  resolve: {
    alias: [
      ...
      // The demo lives OUTSIDE the SDK repo, so bare imports in SDK source
      // would otherwise resolve two copies (demo + sdk). Pin shared runtime
      // deps to the SDK's copies; two ProseMirror instances break the editor.
      { find: /^react-native-enriched$/, replacement: path.join(sdkRoot, 'node_modules/react-native-enriched') },
      { find: /^react-native-render-html$/, replacement: path.join(sdkRoot, 'node_modules/react-native-render-html') },
      { find: /^lodash$/, replacement: path.join(sdkRoot, 'node_modules/lodash') },
